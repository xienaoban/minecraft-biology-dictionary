...
import io.github.xienaoban.biologydictionary.mixin.EatBlockGoalIMixin;
...
public record SheepForceEatGrassSkill() implements EntityTargetedSkill<Sheep> {
    ...
    public static boolean isGrassOrGrassBlock(Sheep entity) {
        ...
        return EatBlockGoalIMixin.biologydictionary$getIsTailGrass().test(level.getBlockState(blockPos))
    ...
    public void serverDo(ServerContext<Sheep> ctx) {
        ...
        if (hasEdibleBlock || hasGrassBlock) {
            ...
            if (hasGrassBlock && level.getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING)) {
            ...
            } else if (!hasGrassBlock && level.getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING)) {
