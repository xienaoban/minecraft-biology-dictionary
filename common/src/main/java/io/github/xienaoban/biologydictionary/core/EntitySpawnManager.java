...
public final class EntitySpawnManager {
    ...
    private static final long STRUCTURE_TEMPLATE_NBT_QUOTA = 128L * 1024 * 1024;
    ...
    private static final Identifier IGNORED_MISSING_TEMPLATE = IdentifierUtils.mc(
        "structure/ancient_city/walls/intact_horizontal_wall_stairs_5.nbt"
    );
    private static final Identifier EMPTY_POOL = IdentifierUtils.mc("empty");
    ...
    private void applyStructureTemplateEntities(Map<Identifier, Identifier> structureStartPools) {
    ...
    private Set<EntityType<?>> analyzeComponentClosure(Identifier componentId) {
    ...
    private StructureAnalysis analyzeTemplate(Identifier templateId) {
        ...
        try {
            ...
            BlockStateVisitor blockStateVisitor = new BlockStateVisitor(
                paletteVisitor.jigsawStates,
                paletteVisitor.spawnerStates
            );
    ...
    private static void parseTemplateNbt(byte[] templateBytes, StreamTagVisitor visitor) throws IOException {
        NbtIo.parseCompressed(
            ...
            visitor,
            NbtAccounter.create(STRUCTURE_TEMPLATE_NBT_QUOTA)
        );
    }
    ...
    private static final class TemplateVisitor extends AbstractTemplateVisitor {
        ...
        public ValueResult visit(String string) {
            if (readingEntityId) {
                EntityType<?> entityType = EntityUtils.getEntityType(string);
                if (entityType != null) {
                    entities.add(entityType);
                }
                readingEntityId = false;
            } else if (readingPool) {
                Identifier poolLoc = IdentifierUtils.fromStringOrNull(string);
        ...
        public ValueResult visitList(TagType<?> tagType, int i) {
            Context context = contextStack.peek();
            if ((context == Context.ENTITIES_LIST
                || context == Context.BLOCKS_LIST
                || context == Context.SPAWN_POTENTIALS_LIST)
    ...
    private void applyDataPackOverrides() {
        ...
        for (Map.Entry<Identifier, List<Resource>> entry : stacks.entrySet()) {
            ...
            Identifier entityId = IdentifierUtils.fromStringOrNull(entityStr.replace('.', ':'));
            EntityType<?> entityType = entityId == null ? null : EntityUtils.getEntityType(entityId);
            if (entityType == null) {
    ...
    private static final class BuildContext {
        ...
        private final Map<Identifier, Set<EntityType<?>>> poolClosureCache = new HashMap<>();
