...
public class BeehiveScreen extends ElementScreen {
    ...
    protected void render(ScreenRenderingContext ctx) {
        ...
        for (int i = 0; i < beeCnt; ++i) {
            ...
            BeeInfo bee = bees[i];
            float beeScale = EntityUtils.isBaby(bee.entity) ? 0.6F : 1F;
            ...
            int beeTop = y - (EntityUtils.isBaby(bee.entity) ? 20 : 25);
            if (customName != null) {
            ...
            if (ctx.getMouseX() > x - 10 && ctx.getMouseX() < x + 10 && ctx.getMouseY() > beeTop && ctx.getMouseY() < y) {
                hoveredBeeTooltip = Arrays.asList(
                        EntityUtils.getNameText(bee.entity),
        ...
        ctx.renderCenteredText(EntityUtils.getEntityTypeNameText(EntityType.BEE), color, ctx.getZ(), LATTICES[6][0] + lw + 16.5F, LATTICES[6][1] + lh + 16);
