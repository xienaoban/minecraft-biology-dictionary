    ...
    public void clientReceive(ClientNetApi.Context ctx) {
        @ClientOnly final class CO { static void receive(
            ...
            InventoryStealingMenu menu = new InventoryStealingMenu(
                    packet.counter(), PlayerUtils.getInventory(ctx.player()), livingEntity, container);
            ctx.player().containerMenu = menu;
            ClientUtils.setScreen(ctx.client(), new InventoryStealingScreen(menu, PlayerUtils.getInventory(ctx.player()), livingEntity));
        }}
        CO.receive(this, ctx);
    }
}
