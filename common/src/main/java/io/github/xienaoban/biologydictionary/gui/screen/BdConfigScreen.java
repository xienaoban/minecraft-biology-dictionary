...
import io.github.xienaoban.biologydictionary.core.widget.TurnPagePercentWidget;
...
public class BdConfigScreen extends AbstractBiologyDictionaryScreen {
    ...
    private void initWidgets() {
        ...
        widgets.add(new TurnPagePercentWidget(0));
    ...
    public static class ReloadServerConfigsWidget extends Widget {
        ...
        protected boolean onMouseDown(float mouseX, float mouseY, int button) {
            if (isMouseLeft(button)) {
                ClientUtils.playScreenSound(ClientUtils.getClient(), SoundEvents.WOODEN_BUTTON_CLICK_ON, 1.0F, 0.8F);
                ClientUtils.setScreen(ClientUtils.getClient(), new ChatScreen(COMMAND));
                return true;
            }
            return super.onMouseDown(mouseX, mouseY, button);
        }
