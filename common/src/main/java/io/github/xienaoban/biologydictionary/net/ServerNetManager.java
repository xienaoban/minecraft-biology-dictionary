...
public final class ServerNetManager {
    ...
    public static void sendSharedDiscoveryIncremental(ServerPlayer player, String discovererName,
                                                      EntityType<?> entityType, DiscoveryRecord record) {
        ServerNetApi.send(player, new SendSharedDiscoveryIncrementalPacket(discovererName, entityType, record));
    }
    ...
    public static void replyDictionaryDiscoveryRecords(ServerPlayer player,
                                                       Map<EntityType<?>, DiscoveryRecord> discoveries) {
        ServerNetApi.send(player, new ReplyBiologyDictionaryDiscoveryFullPacket(discoveries));
    }

    public static void replyHighlightEntitiesSkill(ServerPlayer player, boolean allowed, EntityType<?> entityType,
                                                   float radius) {
        ServerNetApi.send(player, new ReplyHighlightEntitiesPacket(allowed, entityType, radius));
    }

    public static void replyInventoryStealingScreen(ServerPlayer player, int counter, Entity entity,
                                                    Container container) {
