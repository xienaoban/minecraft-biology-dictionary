...
`DiscoveryRecord` 携带发现者 `discoverer` UUID、`global`（派生的全局视图记录为 true）与 `shareChain`（仅供显式的玩家间主动分享使用）。玩家名字不保存在记录中，需要展示时另行解析，能查到名字则显示名字，查不到则显示 UUID。全局视图记录保留原始发现时间与来源，且 `shareChain` 永远为空；全局共享不会读取或写入传播链。
