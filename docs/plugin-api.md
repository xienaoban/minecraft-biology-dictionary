...
`DiscoveryRecord` carries the discoverer `discoverer` UUID, `global` (true for derived global view records) and `shareChain` (reserved exclusively for explicit player-to-player sharing). Player names are not stored in records; they are resolved separately for display when available. Global view records keep the original discovery time and source, and always have an empty `shareChain`; global sharing never reads or writes that chain.
