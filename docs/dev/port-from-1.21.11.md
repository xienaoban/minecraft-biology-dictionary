...
本分支来自 1.21.11 参考项目，移植审计按单词级源码差异执行：

- 所有特性、源码文件、资源文件、测试、文档和构建相关项目文件都必须已移植，或在 26.1.2 中有明确的新方案替代。
- 对于没有 Minecraft 26.1.2 API、加载器架构、official mappings、跨平台模板或构建系统差异理由的代码，应尽可能与 1.21.11 一字不动；注释、局部变量名、方法顺序、空行、换行、缩进、字面量写法等语法风格也不应无故改变。
- 26.1.2 与 1.21.11 的 Minecraft API、平台加载器和项目架构差异很大，必要适配必须保留；但适配面应尽量收窄，避免把格式化、重构、行为改动混入移植。
- 每个保留的差异都应能归入明确类别：去 Architectury、Fabric/NeoForge 新平台模板、MC 26.1.2 API/命名变化、official mappings、Java/Gradle 版本变化、已记录的新实现方案或明确的 bug fix。
- 发现未移植内容、无理由漂移或行为回归时，优先按 1.21.11 源实现恢复，再做最小必要 26.1.2 适配；如果需要重写、裁剪或扩大适配面，必须记录原因。
- 审计过程中一边修改一边记录：修过什么、为什么修、哪些差异确认合理、哪些点仍需后续验证。
