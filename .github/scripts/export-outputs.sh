...
#   both-jars      - validate MOD_JAR_FABRIC and MOD_JAR_NEOFORGE separately
case "$1" in
  both-jars)
    MISSING=()
    ...
    if [ -z "$MOD_JAR_NEOFORGE" ]; then
      MISSING+=("MOD_JAR_NEOFORGE")
    fi
    if [ ${#MISSING[@]} -gt 0 ]; then
      echo "Error: Missing required environment variables: ${MISSING[*]}"
      exit 1
    ...
    echo "mod_jar_neoforge=$MOD_JAR_NEOFORGE" >> "$GITHUB_OUTPUT"
    echo "MOD_JAR_FABRIC=$MOD_JAR_FABRIC" >> "$GITHUB_ENV"
    echo "MOD_JAR_NEOFORGE=$MOD_JAR_NEOFORGE" >> "$GITHUB_ENV"
    echo "Validated: MOD_JAR_FABRIC=$MOD_JAR_FABRIC, MOD_JAR_NEOFORGE=$MOD_JAR_NEOFORGE"
    ;;
  no-jar-name)
